...
| A5 | The ROM stays within a single 32KB bank, no MBC, no cartridge RAM/battery. | MSTR-001 C2, §4 non-goals; R106/R112 (confirmed: no MBC in use, header cart-type `0x00`, `test_rom.py` T1.1/T1.4 assert this on every build). | The generation engine's data/code genuinely outgrows 32KB — MSTR-001 §4 already names this exact trigger. `BL-0020`'s multi-scheme architecture question (once designed by `03-architecture-design-synthesis`) is the most likely near-term source of ROM-budget pressure this project has on record; `03-architecture-design-synthesis` should explicitly check projected data-table growth against this ceiling when it designs the combination mechanism, not assume headroom. |
