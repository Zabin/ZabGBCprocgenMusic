...
| A5 | **Reopened 2026-07-22, no longer treated as confirmed** — was: "the ROM stays within a single 32KB bank, no MBC, no cartridge RAM/battery." | Was based on MSTR-001 C1/C2/§4 v1.0-v1.1 non-goals; R106/R112 confirmed the *current shipped* ROM is single-bank, cart-type `0x00` — that observation about today's tree still stands, but it is no longer read as a forward commitment. | **Trigger already fired** — the project owner directly named the single-bank/no-save framing as an arbitrary decision mistaken for a firm one ("Do not limit to a single bank ceiling. Do not discount saves.") and MSTR-001 v1.2 removed both as non-goals (§4/§9). This assumption is now explicitly under active reconsideration, pending the research MSTR-001 §9 commissions (`02-research-gbc-hardware` on MBC/SRAM hardware facts, `02-research-tooling-and-testing` on multi-bank build/verify impact) — not to be silently re-assumed true or false before that research lands. |
