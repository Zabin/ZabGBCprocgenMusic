...
| FEAT-1160 | FS-115 | [fs-115-arpeggio-rerooted-gated-varied.md](fs-115-arpeggio-rerooted-gated-varied.md) | ✅ Authored 2026-08-21 (`BL-0127`/`ADS-108` §12 + `ADR-0005`, full 20-field spec). The **completion of `FS-114`**, not a successor to it: `FS-114` places the harmony voice on a chord tone at 100 % of its onsets and `IP-1060`'s arpeggio — authored when there was no harmony, to *fake* one — then moves it off that chord for more than half the frames it sounds, taking strong-beat harsh intervals from 12.0 % on the notes selected to **25.7 % on the pitch actually heard**. Specifies the arpeggio re-rooted onto the shared chord, gated so only a chord tone arpeggiates (a passing tone sustains), varied by a per-onset draw whose no-arpeggio member makes the gate and the variation one mechanism, and required to get **cheaper** per frame (`NFR-1280`). Carries **Risk R1 (High, `BL-0130`)** prominently: `FR-1150`'s portamento and `FR-1140`'s vibrato are produced *by this very routine's* per-frame rewrite, so implementing the gate as *skip the write* would delete both on every weak melody onset with the whole suite still green — hence new acceptance criterion **A5**, a check that does not exist today. Three Open Questions, all genuinely upstream: the figure table's contents (`09-content-review`'s ear), rate variation (`BL-0129`'s deferred trigger), and pulse B's octave (unblocked by this feature, deliberately not exercised by it) |
