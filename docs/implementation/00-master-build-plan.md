...
| IP-8030 | Content module decomposition — extract tile pixel bytes/palettes (`visuals.py`) and scale/tempo/style/song/motif/rhythm tables (`music_engine.py`) into three new dependency-free modules (`tiles.py`/`patterns.py`/`music_data.py`), restoring the decomposition `GDS-03`/`GDS-09` always described; byte-identical-ROM equivalence contract (Python-level relocation only) | `BL-0089` (Medium); named critical-path prerequisite immediately before roadmap R12.5 (`01-release-plan.md` §2.5) | **COMPLETE** 2026-08-17 — see [IP-8030](packages/IP-8030-content-module-decomposition.md). ROM byte-identical to pre-refactor baseline (SHA-256 `81d689bd...`), full suite 154/154 unchanged. Awaiting `09-package-verification` (fresh session). |
...
| IP-1140 | **Harmonic coordination via a shared chord context** — one shared `CHORD_IDX` in WRAM every pitched channel reads at its own onset; a hand-authored per-scale `CHORD_TABLE` of scale degrees (not runtime third-stacking, which is *incorrect* against the shipped 8-entry `AND 0x07` degree space); a 4-chord sparse tonic-biased weighted `CHORD_TRANSITION`; harmonic rhythm counted in **pulse-A onsets, not frames** (N=4), with the clock inside pulse A's **existing** onset branch — **no new `engine_tick` call, nothing unconditional per frame**; bass = root/fifth alternating, melody = chord tone on strong onsets and ±1 step on weak, harmony = chord tone an octave below (pulse B's `octave_delta` 0→-1). 3 new WRAM bytes (`0xC077`-`0xC079`), 4 new ROM tables (72 B). Bad-zone recovery kept and made scope-aware (`FR-1590`): dissonance no longer overrides a strong-onset chord tone; stuck and overload unchanged. **The one package in this plan that deliberately changes the shipped boot sound.** Per `ADS-108` §11/D13 + `ADR-0004` (superseding `ADR-0003` **before implementation** — no `SCHEME_TABLE` was ever created, no `CHMIX_MASKS` migration performed, `ADR-0001`'s one-bit packing reaffirmed in full), chord-derived selection *replaces* the default scheme's note selection rather than sitting beside it as a third scheme. `T5`/`T6`-class ±1-walk assertions are re-authored **in this package**, not deferred | `FS-114`/`FEAT-1150`, `BL-0119`; `FR-1500`...`FR-1560`, `FR-1580` (as amended), `FR-1590`, `NFR-1240`...`NFR-1270`. **`FR-1570` withdrawn unimplemented — not covered, must not be claimed.** `FR-1180` unchanged | **VERIFIED** 2026-08-21 — [package](packages/IP-1140-harmonic-coordination.md), [`VR-1140`](verification/VR-1140-harmonic-coordination.md) (genuinely fresh session; the standing G3 grant never waived this and did not need to). Verification re-measured the acceptance figures from scratch at the **PSG-write output boundary** rather than accepting any recorded number: aggregate harsh vertical intervals **32.0% → 23.7%**, strong-beat **33.2% → 22.4%** on sounding pitch — the improvement is real, and `BL-0128`'s correction stands (the banked 12.2% is not reproducible by any windowing). Bad-zone 34→15 and `VIS_ENTRY_LY` 152-153 reproduced the package's own figures exactly; chord transitions hand-derived against the live LFSR 7/7; the per-scale `CHORD_TABLE` confirmed at three non-default scale/octave combinations; `FR-1590` driven 18/18. 4 non-blocking findings (`VR-1140` §6). 171/171 full suite (`T1`-`T22`), ROM 32768 bytes, valid header. **Measured, boot defaults, 3600 frames, strong-beat partitioned per `NFR-1270`: harsh vertical intervals 23.3% → 12.2% on strong-beat sonorities** (`R225` §5f predicted 14.9%), weak-beat 36.6% → 30.6%, aggregate 30.0% → 21.5% — the modest aggregate move alongside the large strong-beat one is exactly the divergence `BL-0122` exists to protect. **⚠️ CORRECTED 2026-08-21 (`BL-0128`) — those figures were measured on the wrong signal and are overstated.** They were sampled at `CUR_DEGREE`, which records what the harmony layer *intends*; `arp_tick` rewrites the frequency register afterwards, every frame, so the pitch that actually sounded was never measured. Re-measured on the same build (`3e635ed`) with a driver that reproduces the figures above exactly on their own basis — so the difference is the instrument, not the run — **on sounding pitch this package's result is strong-beat 25.7%, weak-beat 36.1%, aggregate 30.9%.** The improvement is real but materially smaller than banked, and roughly half of it is overwritten by `IP-1060`'s arpeggio, which `IP-1150` addresses. `NFR-1270` is amended to make sounding pitch the normative basis. Same class of error as `BL-0124` (which corrected *when* to sample) applied to *what* to sample — and it survived that correction. Bad-zone activity 34/121 → 15/121 sampled onsets, with no threshold retuning. `VIS_ENTRY_LY` on chord-transition frames 152-153, inside VBlank. **Three deviations from `FS-114`, each measured rather than assumed**: pulse B's octave move withheld (would be undone by `arp_tick`, and teaching `arp_tick` the offset costs unconditional per-frame instructions — `NFR-1240`/`BL-0113`); `harmony_pick` replaced by `slot_next` (two independent draws put pulse A and pulse B in unison 28% of the time); `passing_table` added (reusing `DELTA_TABLE`'s 50%-hold weighting on weak onsets produced a leap-then-hold melody, measured as directional run length falling 1.86 → 1.26). `T5.5`/`T6.3`/`T6.4` re-authored, not loosened — `T5.5`'s narrowed assertion caught a genuine `CHORD_TOGGLE` initialization off-by-one a loosened version would have missed | G3 granted 2026-08-20 (see below). Acceptance is **not** a green suite: `NFR-1270`'s strong-beat-partitioned interval measurement against the immediately-preceding commit's ROM, plus `09-content-review`'s holistic dimension (`R224` §7a) |
| IP-1150 | **The arpeggio re-rooted, gated and varied** — `_emit_arpeggio_tick` stops adding `[0,2,4,2]` `AND 0x07` scale-degree offsets to `CUR_DEGREE` (i.e. to the chord tone `IP-1140`'s harmony just chose, stacking a second differently-rooted triad on the engine's own chord) and instead plays back a small per-channel cache **resolved at the note's own onset** from tones of the currently-sounding chord. The figure is **drawn per onset** from a weighted pattern table whose reserved `SUSTAIN` value makes the gate (`FR-1600`: only a chord tone arpeggiates — the melody's weak-onset passing tone sustains) and the variation (`FR-1610`) **one mechanism rather than two branches**. Because the cache is resolved at the onset, `arp_tick` loses its whole `SCALE_IDX`/`OCTAVE_IDX`→`ptr_table` address resolution — so per-frame cost must **strictly decrease** (`NFR-1280`), the first head-room actually returned since `R101` §8.5 measured the budget exhausted, absorbing `BL-0125`. Answers the project owner's own listening report (`BL-0127`, *"constant repeated arpeggios"*), measured: pulse B sits on a chord tone at 100% of its onsets yet only 46.4% of sounding pulse frames are chord tones, and strong-beat harsh intervals read 12.0% on intent vs **25.7% on sounding pitch**. Retirement was weighed at `03` as the strongest rival and rejected on evidence; it survives as a one-table-edit fallback (an all-`SUSTAIN` table *is* retirement). **The boot sound changes again, deliberately.** `music_engine.py` + `music_data.py` + `test_rom.py`; new `T23`, `T11` re-authored, `T19` gains an arpeggio-onset frame class | `FS-115`/`FEAT-1160` | **`VERIFIED`** 2026-08-21 — [`VR-1150`](verification/VR-1150-arpeggio-rerooted-gated-varied.md), genuinely fresh session. **Verification hand-derived the whole articulation chain** (`ARP_PATTERN_PICK` → `ARP_PATTERNS` → `CHORD_TABLE` → note table) and matched the engine's resolved cache **400/400 byte-identical across all four scales and the octave-0 floor**; **disproved `BL-0130` at the register-write boundary** (64/64 sustained notes still show a varying sounding period, 79/79 degree-changing onsets still retrigger at the outgoing pitch — vibrato and portamento are alive, the write never stops); and **re-counted `NFR-1280` from the assembler's own label table at `arp_tick` 156 → 110 bytes per channel (-29.5 %)**, with the double-onset frame class measuring `LY` 151 (risk R2 did not materialize). Boundary re-measurement reproduced the package's own figures. 3 non-blocking findings (`VR-1150` §6). Original implementation record follows: — **187/187 (`T1`-`T23`)**, ROM 32768 bytes valid header, 5420/32768 ROM used (27348 free). **Measured on SOUNDING pitch** (`BL-0128`'s corrected basis, boot defaults, 3600 frames, vs `3e635ed`): strong-beat harsh intervals **25.7% → 10.9%**, weak-beat 36.1% → 27.8%, aggregate 30.9% → 19.3%; sounding pulse frames that are chord tones **46.4% → 74.9%**; bad-zone activity 15/121 → **7/121** with no threshold retuning. **On the reported complaint, measured at the source**: distinct articulation shapes **3 → 21**, notes that do not arpeggiate at all **0% → 47.1%** — and 2 of the old 3 shapes were octave-seam wrap variants of the single compiled-in figure, one of which (13.2% of notes) inverted it into a downward sixth. **`NFR-1280` met with margin**: `VIS_ENTRY_LY` on idle frames **152/153 → 151/152**, ~456 cycles of VBlank returned every frame — the first per-frame head-room recovered since `R101` §8.5 declared the budget exhausted. `T22.3`/`T22.4` **re-derived, not loosened** (they correlated a chord change against the same frame's `NOTE_TIMER_PA` reload, a correlation that was never sound and that this package's added onset work exposed — see `BL-0132`); `T11.1` re-authored against the new contract | `IP-1140` — **`VERIFIED` 2026-08-21** ([`VR-1140`](verification/VR-1140-harmonic-coordination.md)), so the argued disposition recorded below is now moot: this package's sole dependency is genuinely verified | G3 granted 2026-08-21 on two cited bases (see below). Acceptance is **not** a green suite: `NFR-1270`'s strong-beat-partitioned measurement **on sounding pitch**, `NFR-1280`'s instruction-count comparison, and `09-content-review`'s ear |
| IP-1160 | **Select becomes a reroll: new material, kept settings** — a Select rising edge keeps its `DIV` LFSR reseed and its bad-zone clear and **stops changing** `TEMPO_IDX`/`OCTAVE_IDX`/`SCALE_IDX`/`DENSITY_IDX`/`CHMIX_IDX`/`DUTY_BIAS`, so the listener's settings survive the press. `init_engine` splits into a **boot-only prologue** (which keeps the label, the call site and its behaviour byte for byte) falling through into a shared body the Select handler calls directly. **The planning finding that defines this package: `init_engine` writes the protected values EIGHT times, not five.** The five `PRESET_*` writes are obvious; a `DUTY_BIAS` clear and — 50 lines further down, under a comment correctly arguing it is harmless *given the writes above it* — a second `TEMPO_IDX`/`DENSITY_IDX` write from `SONG_TABLE[0]` are not. A five-write implementation ships a Select that preserves octave, scale and channel-mix while silently resetting tempo and density, and looks like it works. Surfaced by consulting `GDS-04` §1.2's steering-index writer registry, which has listed *song-form phase transition* as an independent `TEMPO_IDX`/`DENSITY_IDX` writer since 2026-07-26. **No new WRAM field, no new table, no new module; boot deliberately unchanged.** Seven shipped checks assert the removed behaviour and are re-authored (never loosened) — `T17.8` is the canary and is green today *because* the defect is the shipped behaviour. Rides `BL-0139`'s code half | `ADR-0006`; `FR-1070`/`FR-1420`/`FR-1630` **all as amended 2026-08-21**. **`FR-1000` NOT covered** — boot is preserved, not implemented. **No `FS-116`/`FEAT-1170`** — stages 05/06 deliberately skipped, reason recorded in the TWBS | **`COMPLETE`** 2026-08-22 — awaiting fresh-session `09-package-verification` (the standing G3 grant does **not** waive it). Every dependency `VERIFIED` (`IP-0001`/`IP-0005`/`IP-0007`/`IP-1100`/`IP-1130`/`IP-1140`/`IP-1150`), the first package this increment that needed no argued disposition. **Implementation Task 1 re-verified the write count against the tree before any edit: exactly EIGHT writes to protected values in `init_engine` (L1582-1586 the five `PRESET_*`, L1590 `DUTY_BIAS`, L1637-1638 the `SONG_TABLE[0]` `TEMPO_IDX`/`DENSITY_IDX` pair), no ninth — and neither callee (`mood_update`, `arp_resolve_*`) writes one indirectly, so the blocking-drift condition did not fire.** `init_engine` is now a boot-only prologue holding all eight, falling through into `engine_reroll`; `input_map.py`'s Select `CALL` re-points there. `mood_update` deliberately not moved — it still runs after every write to its inputs on the boot path and is a no-op on the reroll path (`BL-0111`'s defect class). **191/191** (`T1`-`T23`), ROM 32768 bytes valid header, **ROM budget delta 0 bytes** (5420/32768 used, unchanged). **Measured at the output boundary on captured PyBoy audio** (never an engine-internal field, per the amended `09-*` rule), at non-default settings (`TEMPO_IDX=7`, `OCTAVE_IDX=2`, `SCALE_IDX=2`, `DENSITY_IDX=2`): (1) the sounded dominant-pitch track differs across **28/40** 250 ms windows across the press — the reroll rerolls; (2) all six protected values read **bit-identical** on the press frame itself; (3) the bad-zone fields (`BAD_ZONE_FLAGS`/`DISSONANCE_SCORE`/`STALE_COUNT_*`) read all-zero on that frame; (4) **boot is unaffected** — WRAM `0xC000`-`0xC09F` byte-identical and the 10 s sounded pitch track identical (40/40 windows) against a build of the immediately-preceding commit `56beec7`. **`VIS_ENTRY_LY` re-measured: 151/152 on idle frames in both builds — `IP-1150`'s recovered scanline is intact**, no regression; 153 appears only on the Start-press frame, `IP-1130`'s own disclosed cost. Seven checks re-authored and inverted (`T5.2`/`T5.3`/`T5.4`, `T5.5`, `T15.6`, `T17.8`, `T18.9`/`T18.10`) plus `T20.11` (`FR-1420`'s no-op), two added (`T5.7` `FR-1630` strengthened, `T5.8` the reroll at the output boundary), one added guard (`T5.4b`, all six values as one tuple), and `t7_noise_density`'s Select-as-reset fixture repaired to a fresh boot per density. `BL-0139`'s code half fixed | `IP-0007` (the reseed, reused unchanged — two-thirds of the capability, shipped 2026-07), `IP-1100` (the phase-0 writes this package must move), `IP-1130` (`BLEND_STEP`'s guard, **kept and strengthened** — a stale blend would now overwrite the *listener's* values, which is exactly what this package promises it will not do), `IP-1140`/`IP-1150` (the chord context and arpeggio caches, which now re-resolve from the listener's scale/octave) | G3 **granted** on two cited bases: the project owner's own direct 2026-08-21 instruction (*"The select-reset does not need to bring it back to the boot default either, just course correct from a bad zone"*) plus his approval of folding in reroll, and the standing increment grant recorded below. Acceptance is **not** a green suite: a live drive at non-default settings confirming all six values survive the press, plus `09-content-review`'s ear on whether it sounds like new music at the same settings |
...
### `IP-1150` — status disposition and G3 authorization (2026-08-21)

**Status is `BLOCKED`, and that is the honest reading of this plan's own rule**, not a formality
waived because the work is wanted: `READY` requires every dependency `VERIFIED`, and `IP-1150`'s
sole real dependency `IP-1140` is `COMPLETE` awaiting a fresh-session `09-package-verification`
that the standing G3 grant explicitly does **not** waive.

**Why the block does not stop the work, recorded rather than assumed.** `IP-1140`'s missing
verification is an outstanding *obligation*, not an outstanding *finding* — nothing has been
reported against it. The two packages touch the same mechanism from opposite sides (selection at
onsets vs. articulation between them) and will be verified as a pair; and `IP-1150` would have to
be re-planned anyway if `IP-1140`'s verification returned findings, so building it first costs
nothing that waiting would save. Above all, the project owner's standing instruction is to *iterate
until it is deemed pleasant and ready for human ears to review* — and `BL-0128` establishes that a
listener today is hearing roughly half of `IP-1140`'s benefit, because `IP-1060`'s arpeggio
overwrites the rest. Recorded here so a later reader sees a deliberate, argued exception rather
than a rule quietly ignored.

**G3 authorization for `IP-1150`: GRANTED 2026-08-21 — two independent bases, both cited.**

1. **The standing grant for the harmonic-coordination increment** (recorded above for `IP-1140`,
   2026-08-20), whose own wording covers *"`IP-1140` and any follow-on remediation package this
   increment's own measurement produces."* `IP-1150` is exactly that: it exists because measuring
   `IP-1140`'s result on the pitch that actually sounds, rather than the pitch it intended, showed
   most of the benefit was being overwritten.
2. **Release-plan coverage** — `FEAT-1160` is bucketed **MVP-critical remediation** in
   `docs/feature-planning/01-release-plan.md` §3 (added 2026-08-21), in this exact shape and scope.

**And the trigger was the project owner's own ear** (*"constant repeated arpeggios"*), which is the
strongest authorization this project recognizes for a change to what the ROM sounds like.

**Limits, unchanged.** No waiver of fresh-session `09-package-verification` (still owed for
`IP-1140` *and* `IP-1150`); no waiver of G5's permanent gates; no extension to any refactoring
package.
