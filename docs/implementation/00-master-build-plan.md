...
| IP-8010 | Remove the vestigial patch-point dicts — `build_engine_asm`/`build_input_asm` construct a `patches = {}` never assigned into, discarded by every caller; delete the dead scaffolding, no behavior change | `BL-0064` (Low); doc-only halves (`GDS-03` language, six `FS-1xx` fields) explicitly routed elsewhere — GDS-03's to `03`, FS fields already clean | **COMPLETE** 2026-07-31 — equivalence proven: ROM byte-identical (SHA-256 `a646a651af...`, pre and post), 129/129 with identical check-name set. Awaits `09-package-verification` (fresh session). |
| IP-8020 | Shared WRAM constants module — extract the 11 constants `visuals.py` duplicates from `music_engine.py` (5 index addresses, 5 `PRESET_*` values, `BAD_ZONE_FLAGS`) into a new dependency-free shared module, preserving the acyclic-import rule; byte-identical-ROM equivalence contract (Python-level relocation only) | `BL-0065` (Low-Medium); explicitly excludes `LY`/`VIS_ENTRY_LY` (not instances of the pattern — see package's own reasoning) | **COMPLETE** 2026-07-31 — new `wram_constants.py` (dependency-free, confirmed no import from either `music_engine.py`/`visuals.py`) is now the canonical definition of all 11 constants; `music_engine.py`/`visuals.py`/`input_map.py` all import from it. Equivalence proven: ROM byte-identical (SHA-256 `a646a651af...`), 129/129 identical check-name set. Awaits `09-package-verification`. |
| IP-1120 | Emotional/Energy Layer — 2 new derived WRAM bytes (`AROUSAL`/`VALENCE`), recomputed only at the 6 write sites that can change their inputs (never per-frame); zero player-visible change, groundwork for roadmap R9's future mood-reactive visualizer work | `FS-112`/`FEAT-1120`, roadmap R7; `FR-1390`...`FR-1420`, `NFR-1170`, `NFR-1180` | **COMPLETE** 2026-07-31 — 142/142 full-suite tests (`T1`-`T20`), authorization `GRANTED 2026-07-31` ("Yes proceed"). Awaits `09-package-verification`. |
...
|---|---|---|---|
| IP-1130 | Genre Blending — interpolates `TEMPO_IDX`/`DENSITY_IDX`/`DUTY_BIAS` between `STYLE_TABLE` rows over 4 discrete steps on a Start press (`SCALE_IDX` hard-switches immediately, unchanged); 7 new independent WRAM bytes (`BLEND_SRC_TEMPO`/`DENSITY`/`DUTY`, `BLEND_STEP`, `BLEND_DELTA_TEMPO`/`DENSITY`/`DUTY`); no new input control | `FS-113`/`FEAT-1130`, roadmap R8; `FR-1470`...`FR-1490`, `NFR-1210`...`NFR-1230`; amends `FR-1240` | **COMPLETE** 2026-08-08 (remediated after `RETURNED` — see [VR-1130](verification/VR-1130-genre-blending.md)), 154/154 full-suite tests (`T1`-`T21`). **`VR-1130`'s Critical finding (F1) root-caused and fixed, not just reproduced**: the original design re-derived each field's delta from `STYLE_TABLE` on every active-blend frame; direct `VIS_ENTRY_LY` instrumentation (not just a plausibility argument) proved this genuinely exceeded the VBlank budget on those frames — `LY` read `0` (mid active-display) rather than `144`-`153`, a real timing defect, not the "harness-sampling artifact" framing the original implementation guessed at. Fixed by precomputing each field's signed delta once, in `_emit_begin_blend` (`BLEND_DELTA_TEMPO`/`DENSITY`/`DUTY`, `0xC074`-`0xC076`), removing the per-frame `STYLE_TABLE` lookup from `_emit_blend_tick` entirely; re-measured `LY` stays `152`-`153` on every active-blend frame after the fix. **Fixing F1 surfaced a second, independent latent defect**, not previously identified by any prior review: `BLEND_STEP` was never explicitly initialized by `init_engine`, so a boot or a Select-reset mid-blend left `blend_tick` free to keep "blending" every subsequent frame using stale/leftover source and delta values, corrupting the freshly-reset `TEMPO_IDX`/`DENSITY_IDX`/`DUTY_BIAS` — confirmed via direct trace (a boot-time spurious mini-blend was always present but self-corrected by coincidence under the original design; my fix's own delta-precompute removed that coincidence, exposing it outright). `init_engine` now explicitly resets `BLEND_STEP` to `4` (settled) on both boot and Select. **One further disclosed, bounded, understood timing effect remains, confirmed via `pyboy` `hook_register` instruction-level tracing (not guessed)**: the *combined* `begin_blend`+`blend_tick` cost on the Start-press frame itself can still exceed one harness `tick()` call's cycle budget, so the blend's first interpolation write can become externally observable one `tick()` call later than the press frame — `_emit_begin_blend`'s own direct writes (`BLEND_SRC_*`/`BLEND_DELTA_*`/`SCALE_IDX`) are unaffected and always land same-frame (independently re-confirmed), and the exact-landing-on-target guarantee (`FR-1480`) holds regardless (confirmed across every transition tested). `T21.3b` added: independently hand-derives a genuine mid-blend value (`BLEND_STEP` neither 0 nor 4) against the shipped ROM, closing the coverage gap that let `F1` ship undetected — the same audit `VR-1130`'s own Verification Checklist required. Also updated `T17.6` (the `IP-1100`/song-form collision check, whose "song-form always wins" assumption predates `IP-1130` and no longer matches the real call order — `blend_tick` now runs after `song_tick`, so `ADS-107`/`FS-113`'s own carried-forward Open Question about this dual-writer interaction is now the observed, correct behavior, not a bug) and `T8.7b` (widened to allow one legitimate same-frame onset re-incrementing `STALE_COUNT_PA` after a Select reset, matching the precedent `T8.7c` already established, exposed by the same boot-timing shift). ROM budget: re-measure owed to the next verification pass (not yet independently re-measured post-remediation). As-shipped blend duration **N=4 frames**, not the package's originally-proposed N=16 — disclosed in this row, `Claude.md`, and `GDS-07`; the package doc's own Risks field is still stale (`VR-1130` F2, unresolved — owned by `07-implementation-planning`, not touched here). The disclosed `T9.3` `NR52`/visuals self-healing race is unchanged by this remediation (unrelated root cause, `visuals.py` still untouched). Awaits `09-package-verification` (fresh session — cannot verify same-session remediation work). |
