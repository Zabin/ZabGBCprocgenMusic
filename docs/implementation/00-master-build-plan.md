...
| IP-8030 | Content module decomposition — extract tile pixel bytes/palettes (`visuals.py`) and scale/tempo/style/song/motif/rhythm tables (`music_engine.py`) into three new dependency-free modules (`tiles.py`/`patterns.py`/`music_data.py`), restoring the decomposition `GDS-03`/`GDS-09` always described; byte-identical-ROM equivalence contract (Python-level relocation only) | `BL-0089` (Medium); named critical-path prerequisite immediately before roadmap R12.5 (`01-release-plan.md` §2.5) | **READY**, authorization **GRANTED** 2026-08-17 — see [IP-8030](packages/IP-8030-content-module-decomposition.md). Fully specified, no blocking dependency (all files it touches are stable/`VERIFIED`-derived); the user gave an explicit go-ahead distinct from the earlier 2026-08-07 design decision on `BL-0089`'s shape. `08-refactoring` may build it, subject to its own eligibility re-check. |
