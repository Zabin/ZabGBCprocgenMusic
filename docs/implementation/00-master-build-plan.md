...
|---|---|---|---|
| IP-1130 | Genre Blending — interpolates `TEMPO_IDX`/`DENSITY_IDX`/`DUTY_BIAS` between `STYLE_TABLE` rows over 4 discrete steps on a Start press (`SCALE_IDX` hard-switches immediately, unchanged); 4 new independent WRAM bytes (`BLEND_SRC_TEMPO`/`DENSITY`/`DUTY`, `BLEND_STEP`); no new input control | `FS-113`/`FEAT-1130`, roadmap R8; `FR-1470`...`FR-1490`, `NFR-1210`...`NFR-1230`; amends `FR-1240` | **IN PROGRESS** — **RETURNED by `09-package-verification` 2026-08-09**, see [VR-1130](verification/VR-1130-genre-blending.md). 153/153 full-suite tests pass and the ROM builds correctly, but `09-package-verification`'s own mandated independent hand-derivation of a genuine mid-blend value (`BLEND_STEP` other than 0/4) exposed a **Critical finding**: `_emit_blend_tick`'s per-frame values for `DENSITY_IDX` (first blend frame only) and `DUTY_BIAS` (every intermediate frame, landing one full frame late) do not match the package's own documented interpolation formula, while `TEMPO_IDX` is always exact — reproduced deterministically across 2 independent style-pair transitions. The field-position-correlated pattern does not fit the "harness-sampling artifact, self-healing, not a defect" framing the implementing commit applied (borrowed from `IP-1110`/`BL-0069`'s precedent, but without that precedent's own falsification step) and is invisible to the entire shipped suite, since `T21`/`T15.1`-`T15.4`/`T4.7`/`T16.8` only ever read blend state at the press frame or after `settle_blend()`'s margin absorbs it. Routed back to `08-code-implementation` to determine root cause (most likely a genuine `_emit_blend_tick` bug) and fix, or independently falsify the harness-artifact framing per the `BL-0069` method before re-asserting it. ROM budget: 28285 bytes free (was 28475 pre-package — 190 bytes used, independently re-measured and confirmed by `VR-1130`). As-shipped blend duration **N=4 frames**, not the package's originally-proposed N=16 — disclosed accurately in this row, `Claude.md`, and `GDS-07`; the package doc's own Risks field is stale (still names only N=16, a Low-Medium finding in `VR-1130`). The disclosed `T9.3` NR52/visuals self-healing race (a pre-existing race newly exposed by this package's added per-frame `blend_tick` call, out of file scope to fix — `visuals.py` untouched) was independently re-confirmed by `VR-1130` over a 1200-frame drive (longest consecutive mismatch run = 1) and its characterization holds. |
