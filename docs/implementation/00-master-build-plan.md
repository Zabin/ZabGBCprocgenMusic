...
| IP | Package | Requirements | Status |
|---|---|---|---|
| IP-1110 | Settings & control visibility — 5 new bar-height indicator tiles (tempo/octave/scale/density/channel-mix), extending `visuals.py`'s existing `update_visuals`/`CHANNEL_CELLS` mechanism | FR-1350...FR-1380, NFR-1140, NFR-1150, NFR-1160 | **COMPLETE** 2026-07-26 — 122/122 full-suite tests (T1-T18). ROM budget: +449 bytes (28528 free, measured via `ADR-0002`'s own `rom.pos` method — larger than the package's original 128-byte tile-data-only estimate, since the per-frame update/init routines' own instruction overhead wasn't counted in that estimate; disclosed here rather than silently absorbed). **Caught and fixed a real regression during implementation**: the package's initial placement of `_emit_update_settings_row` at the very start of `update_visuals` caused the *existing* channel-activity writes to intermittently drop on ordinary frames with no button input (T9.3 failed); reverted to the original tail position, which does not exhibit this regression. **Found (not fixed) a disclosed, narrower, self-healing timing finding**: on the exact frame Select is pressed, `apply_input`'s full `init_engine` reset costs enough extra CPU that this package's own settings-row VRAM writes for that one frame are silently dropped (confirmed independent of block position within `update_visuals` — a pre-existing property of the visualizer's per-frame VRAM-write design, only made observable by this package since it's the first indicator whose value changes across a Select press); the display catches up the very next frame. `test_rom.py`'s `T18.9`/`T18.10` verify the WRAM reset lands on the exact frame and the display catches up within one further frame. |
