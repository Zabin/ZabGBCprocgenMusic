...
| IP-9040 | Wire `mood_update` into genre blending — two `CALL('mood_update')` additions in `_emit_begin_blend`/`_emit_blend_tick` (`music_engine.py`), closing the gap `10-integration-review`'s R7 tranche review found: `IP-1130`'s blend write path never recomputes `AROUSAL`/`VALENCE`, silently violating `FR-1410`. No new WRAM, no new routine. Folds in 4 already-`SCHEDULED` `VR-1130` doc corrections (`BL-0106`-`0109`) | `BL-0111` (Medium-High); folds in `BL-0106`/`BL-0107`/`BL-0108`/`BL-0109` (doc-only) | **BLOCKED** 2026-08-17 — see the package doc's own Blocking Report. G3 was granted (pre-authorized conformance-remediation path, both `CALL('mood_update')` sites implemented correctly per a corrected placement — see report), but independent `VIS_ENTRY_LY` measurement on an active-blend frame with the change in place confirmed the package's own named central risk: a genuine VBlank-budget regression (`VIS_ENTRY_LY` read 0-1, outside 144-153). All code/test changes reverted, nothing partial committed. Needs re-scoping by `07-implementation-planning` (v2) before any further build attempt. Dependencies (`IP-1120`, `IP-1130`) both `VERIFIED`, unaffected. |
