...
| IP-9030 | [VRAM write-integrity detection (`BL-0069`)](IP-9030-vram-write-integrity-detection.md) | **BLOCKED** 2026-07-31 — stage 08 took the package's own measurement and it **falsified the package's premise**: PyBoy models no mode-3 VRAM gating, no write is ever dropped, and the symptom is a `pb.tick()` observation-window artifact uniform across all frame classes. See the package's own [Blocking Report](IP-9030-vram-write-integrity-detection.md#blocking-report--2026-07-31-08-code-implementation-run-102). Needs re-scoping by `07`. |
