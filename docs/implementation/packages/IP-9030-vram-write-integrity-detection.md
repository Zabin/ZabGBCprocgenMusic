...
| **Definition of Done** | `VIS_ENTRY_LY` is written every frame and `T19` exists and passes, asserting 144-153 across all five frame classes. The measured values are recorded in the Implementation Summary **and** in `T19`'s own comments. `T17.6` covers all three phase-transition boundaries; `T18.8`-`T18.10` cover ≥2 pre-Select sequences. All four falsified-wording locations are corrected (`T18.10`'s name, `visuals.py`'s comment block, `Claude.md`, `IP-1110`'s doc). `IP-1080`'s DoD wording is corrected. Full suite passes; ROM builds to exactly 32768 bytes with a valid header. |
