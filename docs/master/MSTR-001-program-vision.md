...
- **Document ID:** MSTR-001 · **Version:** 1.0 · **Status:** ✅ Authored (from-scratch increment
  — no shipped ROM exists yet; this vision is the origin of the project, not a restatement of
  existing code)
- **Date:** 2026-07-21 · **Owned by:** `01-vision` skill
...
1. **Players/listeners:** anyone who wants to carry a small, always-different, hands-on generative
   ...
   bad), not a player failure state (nothing is lost, Select always recovers it).
...
| C5 | **A "bad zone" exists and is recoverable.** The generator must be able to reach a state that is musically bad by some concrete, measurable definition, and **Select** must deterministically return it to a known-good starting state. The exact detection metric (candidates: dissonance interval scoring, note/pattern repetition detection, per-channel write-rate/overload tracking, or a combination) is **not decided here** — delegated to `03-architecture-design-synthesis`/`04-requirements-engineering`, same pattern as C4. | Directly requested by the project owner; this vision fixes only that the mechanic must exist and must be testable (a PyBoy-driven sequence can be asserted to enter and recover from it). |
